...
class PdfExtractionProcessor {
  ...
  Future<PdfExtractionResult> extractTextFromPdf({
    required filePicker.PlatformFile file,
  ...
  Future<String> _extractTextNatively(
    ...
    try {
      ...
      final document = syncfusionFlutterPdf.PdfDocument(inputBytes: pdfBytes);
      ...
      for (int i = 0; i < pageCount; i++) {
        ...
        final pageText = syncfusionFlutterPdf.PdfTextExtractor(document).extractText(startPageIndex: i);
  ...
  Future<int> _getPageCount(filePicker.PlatformFile file) async {
    try {
      if (file.path == null) {
        return 0;
      }
